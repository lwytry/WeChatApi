...
func checkCreateApp(t *testing.T) *App {
	...
	app, err := testmanager.CreateApp(appInitConf)
	checkApp(t, &app)
	checkInfo(t, err)
	return &app
}
...
func checkGetApp(t *testing.T, appID string) {
	app, err := testmanager.GetApp(appID)
	checkApp(t, &app)
	checkInfo(t, err)
}
...
func checkUpdate(t *testing.T, appID string) {
	...
	app, err := testmanager.UpdateApp(appID, appInfo)
	checkInfo(t, err)
...
func checkAllActiveRooms(t *testing.T, appID string) []RoomName {
	rooms, err := testmanager.ListAllActiveRooms(appID, "l")
	checkInfo(t, err)
...
func checkListUser(t *testing.T, appID, roomName string) []User {
	users, err := testmanager.ListUser(appID, roomName)
	checkInfo(t, err)
...
func checkKickUser(t *testing.T, appID, roomName, userID string) {
	err := testmanager.KickUser(appID, roomName, userID)
...
func checkRoomToken(t *testing.T, appID string) {
	roomName := "sdhfuexx"
	userID := "ghjkdfie"
	token, err := testmanager.GetRoomToken(RoomAccess{AppID: appID, RoomName: roomName, UserID: userID, ExpireAt: time.Now().Unix() + 3600})
...
func checkDel(t *testing.T, appID string) {
	err := testmanager.DeleteApp(appID)
	if err != nil {
		t.Error(err)
	}
}
